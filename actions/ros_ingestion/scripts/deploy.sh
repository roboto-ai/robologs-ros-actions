...
echo "Pushing ros_ingestion:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(ros_ingestion:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push ros_ingestion:latest to Roboto's private registry"
    exit 1
fi

echo "Creating ros_ingestion action"
create_args=(
