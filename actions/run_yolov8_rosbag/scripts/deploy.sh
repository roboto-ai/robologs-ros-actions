...
echo "Pushing run_yolov8_rosbag:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(run_yolov8_rosbag:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push run_yolov8_rosbag:latest to Roboto's private registry"
    exit 1
fi

echo "Creating run_yolov8_rosbag action"
create_args=(
