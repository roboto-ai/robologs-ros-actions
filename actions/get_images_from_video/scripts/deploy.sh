...
echo "Pushing get_images_from_video:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(get_images_from_video:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push get_images_from_video:latest to Roboto's private registry"
    exit 1
fi

echo "Creating get_images_from_video action"
create_args=(
