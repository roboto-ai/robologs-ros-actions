...
echo "Pushing rosbag_to_mcap:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(rosbag_to_mcap:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push rosbag_to_mcap:latest to Roboto's private registry"
    exit 1
fi

echo "Creating rosbag_to_mcap action"
create_args=(
