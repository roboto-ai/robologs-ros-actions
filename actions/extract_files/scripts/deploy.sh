...
echo "Pushing extract_files:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(extract_files:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push extract_files:latest to Roboto's private registry"
    exit 1
fi

echo "Creating extract_files action"
create_args=(
