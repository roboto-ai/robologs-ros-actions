...
echo "Pushing parse_rosout_and_tag:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(parse_rosout_and_tag:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push parse_rosout_and_tag:latest to Roboto's private registry"
    exit 1
fi

echo "Creating parse_rosout_and_tag action"
create_args=(
