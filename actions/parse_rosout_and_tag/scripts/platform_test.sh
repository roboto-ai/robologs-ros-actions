...
EXPECTED_OUTPUT_FILE_NAME="changeset.json"
INPUT_DATA="*bag"
ORG=${ORG}
...
invoke_output=$(roboto actions invoke "$ACTION_NAME" --dataset-id "$dataset_id" --parameter-value KEYWORD_DICT='{"error": "Subscribing"}' --input-data "$INPUT_DATA" --org "$ORG")
...
list_output=$(roboto datasets show -d "$dataset_id")
