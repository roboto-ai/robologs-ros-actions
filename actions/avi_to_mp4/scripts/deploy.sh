...
echo "Pushing avi_to_mp4:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(avi_to_mp4:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push avi_to_mp4:latest to Roboto's private registry"
    exit 1
fi

echo "Creating avi_to_mp4 action"
create_args=(
