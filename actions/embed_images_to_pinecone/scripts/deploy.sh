...
echo "Pushing embed_images_to_pinecone:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(embed_images_to_pinecone:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push embed_images_to_pinecone:latest to Roboto's private registry"
    exit 1
fi

echo "Creating embed_images_to_pinecone action"
create_args=(
