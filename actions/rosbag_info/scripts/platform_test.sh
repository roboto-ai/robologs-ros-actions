...
ACTION_NAME=rosbag_info
INPUT_FILE_PATH="./test/input/test.bag"
EXPECTED_OUTPUT_FILE_NAME="test.bag.json"
INPUT_DATA="*bag"
ORG=${ORG}
...
invoke_output=$(roboto actions invoke "$ACTION_NAME" --dataset-id "$dataset_id" --input-data "$INPUT_DATA" --parameter-value SPLIT=True --org "$ORG")
