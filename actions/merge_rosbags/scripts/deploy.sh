...
echo "Pushing merge_rosbags:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(merge_rosbags:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push merge_rosbags:latest to Roboto's private registry"
    exit 1
fi

echo "Creating merge_rosbags action"
create_args=(
