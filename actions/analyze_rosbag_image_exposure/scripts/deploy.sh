...
echo "Pushing analyze_rosbag_image_exposure:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(analyze_rosbag_image_exposure:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push analyze_rosbag_image_exposure:latest to Roboto's private registry"
    exit 1
fi

echo "Creating analyze_rosbag_image_exposure action"
create_args=(
